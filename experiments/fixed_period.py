...
def main(config_path: str, output_path: str):
    ...
    print(m, l, r)
    periods = np.array(tc['periods'])
    scales = np.arange(tc['clock_scale_low'], tc['clock_scale_high'], tc['clock_scale_step'])

    rows = []
    for clock_scale, period in tqdm(product(scales, periods)):
        execution_budget = period * clock_scale
        hit_chance = empirical_cdf(t, execution_budget)
        ...
        devs.sort()
        rows.append([clock_scale, hit_chance, period, devs[m], devs[l], devs[r]])
    
    df = pd.DataFrame(rows, columns=["clock_scale", "hit_chance", "period", "p99", "p99_lower", "p99_upper"])
